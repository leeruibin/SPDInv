...
def diffusion_step(model, controller, latents, context, t, guidance_scale, **kwargs):
    bs = latents.shape[0]
...
class SourcePromptDisentanglementInversion:
    ...
    def SPDInv_loop(self, latent):
        ...
        for i in range(self.num_ddim_steps):
            ...
            # optimize new latent with ODE equation
    ...
    def SPD_inversion(self, image):
        ...
        SPDInv_latents = self.SPDInv_loop(latent)
    ...
    def __init__(self, model, K_round=25, num_ddim_steps=50, learning_rate=0.001, delta_threshold=5e-6,
        ...
        self.enable_shrehold = enable_shrehold
...
def text2image_ldm_stable(
    ...
    if os.path.exists(kwargs.get('ref_image_path', "")):
        ...
        placeholder_idx = find_token_indices_batch(model.tokenizer, prompt, "*")
...
# %%
def ELITE_inversion_and_edit(
        image_path,
        ref_image_path,
        prompt_src,
        prompt_tar,
        checkpoint_path="checkpoints/global_mapper.pt",
        output_dir='outputs',
        guidance_scale=7.5,
        npi_interp=0,
    ...
    ldm_stable = EliteGlobalPipeline.from_pretrained(
        ...
        mapper_model_path=checkpoint_path,
        no_replace_ca_forward=True).to(device)
    ldm_stable.scheduler = scheduler

    SPD_inversion = SourcePromptDisentanglementInversion(ldm_stable, num_ddim_steps=NUM_DDIM_STEPS, enable_shrehold=True)
    ...
    images, x_t = text2image_ldm_stable(ldm_stable, prompts, controller, latent=x_t,
